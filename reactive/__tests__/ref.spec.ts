...
import { isRef, Ref, ref, shallowRef, toRef, toRefs, triggerRef, unref } from '../ref'
import { isReactive, isReadonly, isShallow, reactive } from '../reactive'
import { effect } from '../effect'
import { describe, it, test, expect, vi } from 'vitest'

describe.skip('reactivity/ref', () => {
  ...
  test('should work like a normal property when nested in a reactive object', () => {
    ...
    const assertDummiesEqualTo = (val: number) =>
      [dummy1, dummy2].forEach(dummy =>{
        console.log('val', val)
        return expect(dummy).toBe(val)
      })
  ...
  it('should NOT unwrap ref types nested inside arrays', () => {
    ...
    expect((arr[1] as Ref<any>).value).toBe(3)
  })
  ...
  test('should keep symbols', () => {
    ...
    keys.forEach(key => {
      console.log('key', key, objRef.value[key])
